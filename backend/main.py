...
TOOLS_LIST = [
    ...
    {"name": "get_node_info", "description": "Get detailed info about a page or element. If nodeId is omitted, returns the current page/artboard info. For pages: kind, id, name, x, y, width, height, backgroundColor, childIds, childCount, elementCount. For elements: kind, id, name, tag, type, parentId, pageId, style, text, textSnippet, children, childIds, childCount, geometry (x, y, width, height). Invalid nodeId returns an error.", "inputSchema": {"type": "object", "properties": {"nodeId": {"type": "string", "description": "Optional. Page or element ID. Omit to use current page."}}}},
    ...
    {"name": "write_html", "description": "Write nested HTML or inline SVG into a document with targeted mode support. Supports rich chart and diagram markup with inline CSS and SVG depth details such as gradients, filters, masks, clipPath, nested groups, paths, text, and foreignObject. Single-artboard documents may omit pageId and targetNodeId; multi-artboard documents must provide pageId or targetNodeId. Returns created element array, count, full document state, and deleted IDs when mode is replace.", "inputSchema": {"type": "object", "properties": {"html": {"type": "string", "description": "Non-empty HTML/SVG string with inline CSS styles. When the markup has one top-level wrapper, that wrapper fills the artboard automatically; no need to set position/width/height on it. Include visual depth directly in initial chart or diagram markup where appropriate: layered backgrounds, shadows, borders, subtle gradients, highlights, texture, and surface treatment. If parsing produces zero elements, returns success with created: [] and count: 0."}, "targetNodeId": {"type": "string", "description": "Target page or element ID. If targeting an element, the mode operates on that element. If targeting a page, the mode operates on the page root. Multi-artboard documents require pageId or targetNodeId."}, "pageId": {"type": "string", "description": "Target page ID, used when targetNodeId is not provided. Single-artboard documents may omit pageId/targetNodeId; multi-artboard documents must provide pageId or targetNodeId."}, "mode": {"type": "string", "description": "One of: 'append' (default) - add as children of target, 'replace-children' - remove target's children then append, 'replace' - replace target element itself with new content"}}, "required": ["html"]}},
    {"name": "duplicate_nodes", "description": "Duplicate full element subtrees and return per-root descendant ID maps. nodeIds must be a non-empty array. Missing nodes are reported in errors; valid nodes still duplicate. offsetX and offsetY default to 20 and apply only to the cloned root's positional style, not descendants. Response returns success, duplicated array (each: sourceNodeId, newNodeId, pageId, node, created, createdIds, descendantIdMap), duplicatedCount, and errors.", "inputSchema": {"type": "object", "properties": {"nodeIds": {"type": "array", "items": {"type": "string"}, "description": "Element node IDs to duplicate. Required, non-empty."}, "offsetX": {"type": "number", "description": "Horizontal offset applied to cloned root's positional style only. Default 20."}, "offsetY": {"type": "number", "description": "Vertical offset applied to cloned root's positional style only. Default 20."}}, "required": ["nodeIds"]}},
    {"name": "update_styles", "description": "Update CSS styles on one or more elements (or artboard properties when nodeId matches a page ID). Deep-merges styles into existing element styles. Also supports optional style key removal via removeStyleKeys. Response returns success and updated array with per-node results; only successful updates appear in updated; missing or unmatched nodes are silently skipped.", "inputSchema": {"type": "object", "properties": {"nodeIds": {"type": "array", "items": {"type": "string"}, "description": "Element node IDs (or artboard/page IDs) to update. Multi-node supported."}, "styles": {"type": "object", "description": "CSS keys and values to deep-merge into each target element's existing style. For artboard targets (page IDs), only recognized keys (x, y, width, height, backgroundColor, name) are applied."}, "removeStyleKeys": {"type": "array", "items": {"type": "string"}, "description": "Optional style keys to remove from element styles after applying styles. Must be an array of non-empty strings when provided. Applies to elements only; never reaches artboard/page updates. Example: ['left', 'top']."}}, "required": ["nodeIds", "styles"]}},
    {"name": "set_text_content", "description": "Set text content on one or more elements. Replaces the element's text field while preserving children, style, and identity. Missing or invalid nodes are silently skipped. Response returns updated element array.", "inputSchema": {"type": "object", "properties": {"nodeIds": {"type": "array", "items": {"type": "string"}, "description": "Element node IDs to update. Multi-node supported. Missing nodes are silently skipped."}, "text": {"type": "string", "description": "New text content to set on each target element. Replaces existing text; children and style are preserved. Use empty string (\"\") to clear text content."}}, "required": ["nodeIds", "text"]}},
    ...
    {"name": "get_html", "description": "Get clean exported HTML for the current page, a page, or an element subtree. Response returns success, kind (page/element), html, pageId, and nodeId for element exports. Omit nodeId and pageId to export the current page. nodeId may be a page ID (exports that page) or element ID (exports that subtree). pageId validates and constrains the lookup. Invalid pageId or nodeId returns an error.", "inputSchema": {"type": "object", "properties": {"nodeId": {"type": "string", "description": "Optional. Page or element ID. Omit to use current page."}, "pageId": {"type": "string", "description": "Optional. Target page ID."}}}},
    ...
    {"name": "delete_nodes", "description": "Delete element subtrees. Removes each requested node plus all of its descendants, cleans up parent/children references, and supports optional pageId constraint. nodeIds must be a non-empty array. Missing nodes are reported in errors; valid nodes still delete. If both a parent and its descendant are requested, only the parent root is processed and the descendant is deleted as part of that subtree. Response returns success, deleted (flattened list of all deleted IDs), deletedCount, deletedRoots (input order deduped), deletedByRoot (map of root to subtree IDs), and errors. Pages are never deleted.", "inputSchema": {"type": "object", "properties": {"nodeIds": {"type": "array", "items": {"type": "string"}, "description": "Element node IDs to delete. Required, non-empty."}, "pageId": {"type": "string", "description": "Optional page ID. If provided, validates first and constrains deletion to that page."}}, "required": ["nodeIds"]}},
    {"name": "move_nodes", "description": "Move element nodes within the same page (reorder or reparent). Preserves whole subtrees. Same-page only — cross-page moves are rejected. Cycle protection: a node cannot be moved under its own descendant. targetParentId may be a page ID or element ID; if omitted, target is the page root. If pageId is provided, lookup is constrained to that page. index is the insertion position inside the target's children order; omitted or invalid appends. Response returns success, pageId, targetParentId, moved array (each: nodeId, oldParentId, newParentId, oldIndex, newIndex), movedCount, and errors.", "inputSchema": {"type": "object", "properties": {"nodeIds": {"type": "array", "items": {"type": "string"}, "description": "Element node IDs to move. Required, non-empty."}, "targetParentId": {"type": "string", "description": "Target page ID or element ID. If omitted, pageId is used as the page-root target."}, "pageId": {"type": "string", "description": "Optional page ID. If provided, constrains targetParentId lookup to that page; otherwise targetParentId can be any page or element."}, "index": {"type": "number", "description": "Insertion index inside the target's children array (or page-root order). Omitted or invalid → append at end."}}, "required": ["nodeIds"]}},
    ...
    {"name": "get_overflow_report", "description": "Report nodes that overflow their parent or artboard, including overflow amounts per side. Defaults: includeArtboard=true, includeParent=true, minOverflow=1. Omit pageId/nodeId to check the current page. nodeId may target a page (checks all elements) or an element (checks that subtree). minOverflow coerced to number; invalid/negative values fall back to 1. Response returns success, pageId, targetNodeId, checkedNodeCount, overflowCount, overflows array (each item: type, nodeId, pageId, severity, per-side amount, geometry, boundary, message), and summary.", "inputSchema": {"type": "object", "properties": {"pageId": {"type": "string", "description": "Optional page ID."}, "nodeId": {"type": "string", "description": "Optional page or element ID."}, "includeArtboard": {"type": "boolean", "description": "Include artboard boundary overflow checks. Default true."}, "includeParent": {"type": "boolean", "description": "Include parent boundary overflow checks. Default true."}, "minOverflow": {"type": "number", "description": "Minimum pixel overflow to report. Default 1."}}}},
    ...
    {"name": "update_svg_attributes", "description": "Update SVG/XML attributes on an existing SVG node. Writes attrs into the node style dict so export/render paths emit them as SVG attributes.", "inputSchema": {"type": "object", "properties": {"nodeId": {"type": "string", "description": "ID of the SVG element (root or descendant) to update. Must be an SVG element — page IDs and non-SVG element IDs are rejected."}, "attrs": {"type": "object", "description": "SVG/XML attribute names expected by the renderer/exporter, for example viewBox, fill, strokeWidth, clipPath, href, d, id. Must be a non-empty object. Values may be string, number, boolean, or null. Native SVG id is represented as style[\"id\"]. Response returns success, nodeId, updated element, and attrs applied."}}, "required": ["nodeId", "attrs"]}},
...
async def api_create_element(doc_id: str, element: dict, page_id: Optional[str] = None):
    try:
        # Accept legacy element-only payloads as well as {element, pageId} wrappers.
        if isinstance(element.get("element"), dict):
            page_id = element.get("pageId") or element.get("page_id") or page_id
            element = element["element"]
        else:
            page_id = element.pop("pageId", None) or element.pop("page_id", None) or page_id
        result = doc_store.create_element(doc_id, element, page_id)
