...
TOOLS_LIST = [
    ...
    {"name": "write_html", "description": "Write HTML into a document with targeted mode support. Returns created element array, count, full document state, and deleted IDs when mode is replace.", "inputSchema": {"type": "object", "properties": {"html": {"type": "string", "description": "Must be a non-empty string. If parsing produces zero elements, returns success with created: [] and count: 0."}, "targetNodeId": {"type": "string", "description": "Target page or element ID. If targeting an element, the mode operates on that element. If targeting a page, the mode operates on the page root."}, "pageId": {"type": "string", "description": "Target page ID (used only if targetNodeId is not provided). If omitted, uses the current page."}, "mode": {"type": "string", "description": "One of: 'append' (default) — add as children of target, 'replace-children' — remove target's children then append, 'replace' — replace target element itself with new content"}}, "required": ["html"]}},
