...
TOOLS_LIST = [
    ...
    {"name": "write_html", "description": "Write rich HTML or SVG into a document with targeted mode support. Supports inline CSS, nested HTML/SVG, gradients, filters, masks, clipPath, groups, paths, text, foreignObject, and visual depth. Single-artboard documents may omit pageId and targetNodeId; multi-artboard documents must provide pageId or targetNodeId. Returns created element array, count, full document state, and deleted IDs when mode is replace.", "inputSchema": {"type": "object", "properties": {"html": {"type": "string", "description": "Must be a non-empty string. Supports inline CSS and nested HTML/SVG including gradients, filters, masks, clipPath, groups, paths, text, foreignObject, and visual depth. If parsing produces zero elements, returns success with created: [] and count: 0."}, "targetNodeId": {"type": "string", "description": "Target page or element ID. Required with pageId for multi-artboard documents when the other target is omitted. If targeting an element, the mode operates on that element. If targeting a page, the mode operates on the page root."}, "pageId": {"type": "string", "description": "Target page ID, used when targetNodeId is not provided. Single-artboard documents may omit pageId/targetNodeId; multi-artboard documents must provide pageId or targetNodeId."}, "mode": {"type": "string", "description": "One of: 'append' (default) - add as children of target, 'replace-children' - remove target's children then append, 'replace' - replace target element itself with new content"}}, "required": ["html"]}},
